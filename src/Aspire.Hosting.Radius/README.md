...
| `ASPIRERADIUS032` | Publish/Deploy gate | A resource is ambiguously assigned to more than one Radius resource group; it must resolve to exactly one. Keep a single `WithRadiusResourceGroup(...)` call per resource (the last call wins if repeated). A route declared on a child resource (e.g. a database on its server) applies to its parent, so a parent/child (or child/child) group disagreement trips this too. |
| `ASPIRERADIUS033` | Config (call site) | A Radius resource-group name passed to `WithRadiusResourceGroup` is not a single safe path/UCP segment: it is empty/whitespace, or contains a path separator (`/` or `\`), the relative-path token `..`, leading/trailing whitespace or dots, or a control character. The name is used verbatim as both the `groups/<group>/` artifact directory and a UCP-ID segment. |
