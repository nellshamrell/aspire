...
public class MultiCloudPublishTests
{
    ...
    [Fact]
    public void AzureAndAws_ManagedResources_BothRecipes_InOneArtifact()
    {
        ...
        Assert.Contains(AwsCacheRecipe, bicep);
        Assert.DoesNotContain("local-dev/sqldatabases", bicep);
        Assert.DoesNotContain("local-dev/rediscaches", bicep);
    }

    [Fact]
    public void MultiCloud_ComputeStaysKubernetes_AndConnectsToBoth()
    {
        using var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Publish);
        var env = builder.AddRadiusEnvironment("myenv")
            .WithAzureProvider(Sub, Rg, azure => azure.WithWorkloadIdentity(Tenant, Client))
